    ...
    <div class="h-full flex flex-col w-full max-w-[1920px] mx-auto p-4 sm:p-8 relative gap-6">
      ...
      <header class="flex flex-col md:flex-row md:items-center justify-between gap-4 shrink-0">
        ...
        <div class="flex items-center gap-3">
          <button class="px-5 py-2.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 font-bold rounded-xl text-sm hover:bg-slate-50 dark:hover:bg-slate-700 shadow-sm transition-all focus:ring-4 focus:ring-slate-100 dark:focus:ring-slate-800 flex items-center gap-2">
          ...
          <button class="px-5 py-2.5 bg-[#6D28D9] text-white font-bold rounded-xl text-sm hover:bg-[#5b21b6] shadow-lg shadow-purple-900/20 dark:shadow-purple-900/40 transition-all active:scale-95 flex items-center gap-2">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6 shrink-0">
        ...
        <div class="bg-white dark:bg-slate-800/80 p-6 rounded-2xl border border-slate-100 dark:border-slate-700/50 shadow-sm flex flex-col gap-4 relative overflow-hidden group">
          ...
          <div class="flex items-baseline gap-3 z-10">
            <span class="text-4xl font-black text-slate-900 dark:text-white tracking-tight">124</span>
            <span class="px-2 py-1 rounded-md bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 text-xs font-bold">+2 nuevos</span>
          </div>
        </div>
        ...
        <div class="bg-white dark:bg-slate-800/80 p-6 rounded-2xl border border-slate-100 dark:border-slate-700/50 shadow-sm flex flex-col gap-4 relative overflow-hidden group">
          ...
          <div class="flex items-baseline gap-3 z-10">
            <span class="text-4xl font-black text-slate-900 dark:text-white tracking-tight">45</span>
        ...
        <!-- Cobertura -->
        <div class="bg-white dark:bg-slate-800/80 p-6 rounded-2xl border border-slate-100 dark:border-slate-700/50 shadow-sm flex flex-col gap-4 relative overflow-hidden group">
          <div class="absolute right-0 top-0 w-24 h-24 bg-orange-50 dark:bg-orange-900/20 rounded-bl-full -mr-4 -mt-4 transition-transform group-hover:scale-110"></div>
          <div class="flex items-center gap-3 z-10">
            <div class="w-10 h-10 rounded-lg bg-orange-50 dark:bg-orange-900/20 text-orange-600 dark:text-orange-400 flex items-center justify-center">
            ...
            <span class="font-bold text-slate-500 dark:text-slate-400 text-sm">Cobertura General</span>
          </div>
          <div class="flex items-baseline gap-3 z-10">
            <span class="text-4xl font-black text-slate-900 dark:text-white tracking-tight">36%</span>
            <span class="px-2 py-1 rounded-md bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 text-xs font-bold">+1.5% este mes</span>
          </div>
        </div>
      </div>
      ...
      <div class="flex-1 flex gap-6 min-h-0 relative">
        ...
        <div class="flex-1 flex flex-col bg-white dark:bg-slate-800/80 rounded-2xl border border-slate-200 dark:border-slate-700/50 shadow-sm overflow-hidden transition-all duration-300">
           ...
           <div class="p-4 border-b border-slate-100 dark:border-slate-700/50 flex flex-col sm:flex-row gap-4">
              ...
              <div class="flex gap-2">
                 <button class="px-3 py-2 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-lg text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 flex items-center gap-2 transition-colors">
                   Todas las Zonas
                  ...
                  <tbody class="divide-y divide-slate-200 dark:divide-slate-800">
                    <tr 
                      *ngFor="let t of filteredTerritorios()" 
                      ...
                      [class.dark:bg-purple-900/20]="selectedTerritorio()?.id === t.id"
                    >
                       <!-- ID/Name -->
                       <td class="px-6 py-4">
                          <p class="font-bold text-slate-900 dark:text-slate-100 group-hover:text-[#6D28D9] dark:group-hover:text-purple-400 transition-colors">{{ t.id }}</p>
                          <p class="text-xs text-slate-500 dark:text-slate-400 font-medium">{{ t.nombre }}</p>
                       </td>
                       <!-- Zone -->
                       <td class="px-6 py-4 text-sm font-semibold text-slate-600 dark:text-slate-300">{{ t.zona }}</td>
                       <!-- Dwellings -->
                       <td class="px-6 py-4 text-sm font-bold text-slate-700 dark:text-slate-200 tabular-nums">{{ t.viviendas }}</td>
                       <!-- Status -->
                       <td class="px-6 py-4">
                          <span 
                       ...
                       <td class="px-6 py-4">
                          ...
                          <span *ngIf="!t.asignadoA" class="text-xs text-slate-300 dark:text-slate-600 font-medium">-</span>
                       </td>
                       <!-- Actions -->
           ...
           <div class="px-6 py-3 border-t border-slate-100 dark:border-slate-700/50 bg-slate-50/50 dark:bg-slate-800/50 flex items-center justify-between">
              ...
              <div class="flex gap-1">
                 <button class="w-8 h-8 flex items-center justify-center rounded-lg bg-[#5B3C88] text-white text-xs font-bold shadow-md shadow-purple-900/10 dark:shadow-purple-900/40">1</button>
                 <button class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-white dark:hover:bg-slate-700 text-slate-500 dark:text-slate-400 text-xs font-bold transition-colors">2</button>
                 <button class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-white dark:hover:bg-slate-700 text-slate-500 dark:text-slate-400 text-xs font-bold transition-colors">3</button>
              </div>
           </div>
        </div>
        ...
        <div 
          *ngIf="selectedTerritorio()"
          @slideOver
          class="w-full md:w-[400px] shrink-0 bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700/50 shadow-xl overflow-hidden flex flex-col h-full absolute md:relative right-0 z-20"
        >
           <!-- Header -->
           ...
           <div class="flex-1 overflow-y-auto p-6 simple-scrollbar space-y-6">
              ...
              <!-- Map Placeholder -->
              <div class="w-full aspect-video bg-slate-100 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700/50 relative group overflow-hidden cursor-pointer shadow-inner">
                 ...
                 <div class="absolute inset-0 flex items-center justify-center">
                    <button class="bg-white/90 dark:bg-slate-800/90 backdrop-blur-sm px-4 py-2 rounded-lg text-xs font-bold text-slate-700 dark:text-slate-200 shadow-sm flex items-center gap-2 group-hover:bg-white dark:group-hover:bg-slate-800 transition-colors">
              ...
              <div class="bg-purple-50/50 dark:bg-purple-900/20 rounded-xl p-5 border border-purple-100 dark:border-purple-800/30">
                 ...
                 <div *ngIf="selectedTerritorio()!.estado === 'Asignado' && selectedTerritorio()!.asignadoA as assignee; else unassignedState">
                    <div class="flex items-center gap-4 mb-4">
                       <div class="w-12 h-12 rounded-full bg-white dark:bg-slate-800 text-purple-700 dark:text-purple-400 font-bold text-lg flex items-center justify-center shadow-sm border border-purple-100 dark:border-purple-800/50">
                          {{ assignee.nombre.charAt(0) }}{{ assignee.nombre.split(' ')[1]?.charAt(0) }}
                       </div>
                       <div>
                          <p class="font-bold text-slate-800 dark:text-slate-200">{{ assignee.nombre }}</p>
                          <p class="text-xs text-slate-500 dark:text-slate-400 font-medium">Asignado: {{ assignee.fecha }}</p>
                       </div>
                    </div>
                    ...
                    <div class="space-y-1.5">
                       ...
                       <div class="h-2 w-full bg-purple-200/50 dark:bg-purple-900/40 rounded-full overflow-hidden">
                 ...
                 <ng-template #unassignedState>
                    <div class="text-center py-4">
                       <p class="text-sm font-semibold text-slate-500 dark:text-slate-400">Este territorio no tiene asignación actual.</p>
                       <button class="mt-3 px-4 py-2 bg-white dark:bg-slate-800 border border-purple-200 dark:border-purple-800/50 text-purple-700 dark:text-purple-400 text-xs font-bold rounded-lg hover:bg-purple-50 dark:hover:bg-purple-900/20 transition-colors">
                          Asignar Ahora
                       </button>
                    </div>
                 </ng-template>
           ...
           <div class="p-6 border-t border-slate-100 dark:border-slate-700/50 bg-white dark:bg-slate-800 grid grid-cols-1 gap-3">
              <button class="w-full py-3.5 bg-[#5B3C88] hover:bg-[#4a2f73] text-white rounded-xl font-bold text-sm shadow-lg shadow-purple-900/10 dark:shadow-purple-900/40 transition-all flex items-center justify-center gap-2">
              ...
              <div class="grid grid-cols-2 gap-3">
                 <button class="py-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700/50 text-slate-700 dark:text-slate-200 rounded-xl font-bold text-sm hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors flex items-center justify-center gap-2">
